"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WarehouseService = void 0;
const common_1 = require("@nestjs/common");
const sequelize_1 = require("@nestjs/sequelize");
const warehouse_model_1 = require("./warehouse.model");
let WarehouseService = class WarehouseService {
    constructor(warehouseModel) {
        this.warehouseModel = warehouseModel;
    }
    async create(warehouseName, maxStockAmount) {
        const client = await warehouse_model_1.Warehouse.create({ warehouseName, maxStockAmount });
        return client.toJSON();
    }
    async find(clientID) {
        const warehouses = await warehouse_model_1.Warehouse.findAll({ where: { clientID } });
        return warehouses.map((warehouse) => warehouse.toJSON());
    }
    async update(warehouseID, attrs) {
        const warehouse = await warehouse_model_1.Warehouse.findByPk(warehouseID);
        if (!warehouse) {
            throw new common_1.NotFoundException('warehouse not found');
        }
        await warehouse.update(attrs);
        return warehouse.toJSON();
    }
    async remove(warehouseID) {
        const warehouse = await warehouse_model_1.Warehouse.findByPk(warehouseID);
        if (!warehouse) {
            throw new common_1.NotFoundException('warehouse not found');
        }
        await warehouse.destroy();
        return warehouse.toJSON();
    }
};
WarehouseService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, sequelize_1.InjectModel)(warehouse_model_1.Warehouse)),
    __metadata("design:paramtypes", [Object])
], WarehouseService);
exports.WarehouseService = WarehouseService;
//# sourceMappingURL=warehouse.service.js.map